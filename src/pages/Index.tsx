...
const getNavLinks = (t: any) => [
    { id: 'vantaggi', href: '#vantaggi-strategici', label: t('advantages') },
    { id: 'servizi', href: '#servizi', label: t('servicesTitle') },
    { id: 'chi-sono', href: 'https://www.linkedin.com/in/marcelo-cavaldid/', label: t('whoAmI'), external: true },
    { id: 'contatti', href: 'https://cal.com/marcecv/30min', label: t('contactMe'), external: true },
];

const getFeatureTabsData = (t: any) => [
    {
        id: 'team',
        title: t('unleashTeamPotential'),
        content: [
            { subtitle: t('timeSaving'), text: t('timeSavingDesc') },
            { subtitle: t('operationalEfficiency'), text: t('operationalEfficiencyDesc') }
        ]
    },
    {
        id: 'marketing',
        title: t('multiplyMarketing'),
        content: [
            { subtitle: t('increaseRevenue'), text: t('increaseRevenueDesc') },
            { subtitle: t('smartDecisions'), text: t('smartDecisionsDesc') }
        ]
    },
    {
        id: 'scalability',
        title: t('unlimitedScalability'),
        content: [
            { subtitle: t('sustainableGrowth'), text: t('sustainableGrowthDesc') },
            { subtitle: t('continuousInnovation'), text: t('continuousInnovationDesc') }
        ]
    }
];
...
const getSolutionsData = (t: any) => [
    {
        id: 'partnership',
        title: t('partnershipAI'),
        content: {
            mainTitle: t('strategyToImplementation'),
            text: t('partnershipDesc')
        }
    },
    {
        id: 'box-ai',
        title: t('boxAIMarketing'),
        content: {
            mainTitle: t('immediateAutomations'),
            text: t('boxDesc'),
            list: [
                { title: t('boxContent'), description: t('boxContentDesc') },
                { title: t('boxManagement'), description: t('boxManagementDesc') },
                { title: t('boxLeadNurturing'), description: t('boxLeadNurturingDesc') }
            ]
        }
    }
];
            ...
            <div className="flex justify-start">
                <div className="max-w-3xl text-left">
                    <h1 className={`text-5xl md:text-7xl font-medium leading-tight transition-colors duration-500 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                        {t('heroTitle')}<br />
                        <span className="text-cyan-500 dark:text-cyan-400">{t('heroTitleHighlight')}<BlinkingCursor /></span>
                    </h1>
                    ...
                    <div className="mt-8">
                        <a href="https://cal.com/marcecv/30min" target="_blank" rel="noopener noreferrer" className={`animated-border-btn ${theme === 'light' ? 'light-mode' : 'dark-mode'}`}>
                           <span className="btn-text">{t('bookCall')}</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-16 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                ...
                <div className={`info-box rounded-lg p-6 md:p-8 text-center shadow-lg border transition-all duration-300 hover:shadow-cyan-400/20 hover:-translate-y-2 ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-neutral-900 border-neutral-700'}`}>
                    <div className="flex flex-col items-center">
                        ...
                        <h3 className={`text-lg md:text-xl font-medium ${theme === 'light' ? 'text-black' : 'text-white'}`}>{t('time')}</h3>
                    </div>
                ...
                <div className={`info-box rounded-lg p-6 md:p-8 text-center shadow-lg border transition-all duration-300 hover:shadow-cyan-400/20 hover:-translate-y-2 ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-neutral-900 border-neutral-700'}`}>
                    <div className="flex flex-col items-center">
                        ...
                        <h3 className={`text-lg md:text-xl font-medium ${theme === 'light' ? 'text-black' : 'text-white'}`}>{t('costs')}</h3>
                    </div>
                    <p className={`mt-4 text-sm md:text-base ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                        {t('costsDescription')}
                    </p>
                ...
                <div className={`info-box rounded-lg p-6 md:p-8 text-center shadow-lg border transition-all duration-300 hover:shadow-cyan-400/20 hover:-translate-y-2 ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-neutral-900 border-neutral-700'}`}>
                    <div className="flex flex-col items-center">
                        ...
                        <h3 className={`text-lg md:text-xl font-medium ${theme === 'light' ? 'text-black' : 'text-white'}`}>{t('scalability')}</h3>
                    </div>
                    <p className={`mt-4 text-sm md:text-base ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>
                        {t('scalabilityDescription')}
                    </p>
                </div>
            </div>
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    <div className="md:col-span-4">
                        ...
                        {/* Language Selector */}
                        <div className="mt-6">
                            <label className="block text-gray-400 text-sm mb-2">{t('selectLanguage')}</label>
                            <Select value={language} onValueChange={(value: 'it' | 'en' | 'es') => setLanguage(value)}>
                                <SelectTrigger className="w-32 bg-neutral-800 border-neutral-700 text-white">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="bg-neutral-800 border-neutral-700">
                                    <SelectItem value="it" className="text-white hover:bg-neutral-700">Italiano</SelectItem>
                                    <SelectItem value="en" className="text-white hover:bg-neutral-700">English</SelectItem>
                                    <SelectItem value="es" className="text-white hover:bg-neutral-700">Español</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                    <div className="md:col-span-4">
                        <h4 className="font-bold text-white mb-4">{t('menu')}</h4>
                ...
                <div className="mt-16 border-t border-neutral-800 pt-8 text-center text-gray-500 text-sm">
                    <p>© {new Date().getFullYear()} ismagit. {t('allRightsReserved')}</p>
                </div>
            </div>
